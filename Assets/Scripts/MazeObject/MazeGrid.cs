    ...
    public MazeGrid(int width, int height, float cellSize, MazeTile defaultTile, MazeModes mode)
    {
        ...
        for (int x = 0; x < gridArray.GetLength(0); x++)
        {
            for (int y = 0; y < gridArray.GetLength(1); y++)
            {
                ...
                {
                    spriteArray[x, y] = CreateTile(map.transform, "maze", GetWorldPosition(x, y) + new Vector3(cellSize, cellSize) * .5f, Camera.main.backgroundColor, 0.95f, 1);
                    SetValue(x, y, defaultTile);
                } 
            }
        } 
